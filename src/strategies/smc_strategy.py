...
Pipeline
--------
G1  Data availability check
G2  H4 bias  (bullish / bearish / neutral)
G3  M15 confluent zones  (OB + FVG, or OB-only fallback)
G4  Price inside a recent zone
G5  M5 CHoCH / BOS confirmation
G6  ML filter gate  (per-symbol XGBoost model)
    → SL / TP calculation → position sizing → Signal

Changes vs previous version
----------------------------
[FIX-ML-1]  MLFilter now receives symbol=symbol so it loads the correct
             per-symbol .pkl file instead of always defaulting to the
             EURUSD model.  This was the root cause of every pair being
             filtered through the EURUSD model regardless of pair.

[FIX-ML-2]  H1 bar count raised 100 → 200 to match the lookback used
             during model training (needed for lagged / rolling features
             to be computed correctly on the first warm-up bars).

[CLEAN-1]   Moved repeated `import logging` calls from inside
             generate_signal() to module level.  No behaviour change.

[CLEAN-2]   Added try/except around MLFilter instantiation so a missing
             or corrupt .pkl file falls back gracefully to the generic
             EURUSD model rather than crashing the scan cycle.
"""
        ...
        if h1 is not None:
            try:
                ...
                if self.ab_test and not self.use_mtl:
                    ...
                    allow, ml_score = ml_filter.should_trade(h1)
